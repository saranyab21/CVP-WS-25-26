...
def parseArgs(parser):
    parser.add_argument('--labels_test', default="icdar17_local_features/icdar17_labels_test.txt",
                        help='contains test images/descriptors to load + labels')
    parser.add_argument('--labels_train', default="icdar17_local_features/icdar17_labels_train.txt",
...
def esvm(encs_test, encs_train, C=1000):
    ...
    """
    label_template = np.zeros(len(encs_train) + 1)
    label_template[0] = 1

    def loop(i):
        # set up labels: 1 for the test sample, 0 for all train samples
        #y = np.zeros(len(encs_train) + 1)
        #y[0] = 1
        y = label_template.copy()
        # stack the test sample on top of the train samples
        X = np.vstack([encs_test[i], encs_train])
        # train linear SVM
        svm = LinearSVC(C=C, dual=False, max_iter=5000)
        svm.fit(X, y)
        # use the SVM weights as the new embedding
        return svm.coef_.reshape(1, -1)

    # Use joblib.Parallel for parallelization with tqdm progress bar
...
if __name__ == '__main__':
    ...
    np.random.seed(42)
    if args.verbose:
        print("Starting pipeline with verbose output.")
